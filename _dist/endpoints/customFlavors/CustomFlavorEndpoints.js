...
import { CUSTOM_FLAVORS_ADD, CUSTOM_FLAVORS_GET_ALL, CUSTOM_FLAVORS_GET_PUBLIC, CUSTOM_FLAVORS_REMOVE, CUSTOM_FLAVORS_UPDATE_VISIBILITY } from "../../@types/ApiBodyFormats/Users.js";
import CustomFlavorManager from "../../sql/CustomFlavorManager.js";
...
export default class CustomFlavorEndpoints {
    ...
    init(app) {
        app.post("/customFlavors/getPublicFlavors", async (req, res) => {
            const body = req.body;
            if (!EndpointUtils.checkAndSend(body, CUSTOM_FLAVORS_GET_PUBLIC, res))
                return;
        ...
        app.post("/customFlavors/add", async (req, res) => {
            const body = req.body;
            if (!EndpointUtils.checkAndSend(body, CUSTOM_FLAVORS_ADD, res))
                return;
            const jwt = body.jwt;
        ...
        app.post("/customFlavors/delete", async (req, res) => {
            const body = req.body;
            if (!EndpointUtils.checkAndSend(body, CUSTOM_FLAVORS_REMOVE, res))
                return;
            const jwt = body.jwt;
            const jwtData = JWTUtils.checkJWTAndResponse(jwt, res);
            if (!jwtData)
        ...
        app.post("/customFlavors/getAll", async (req, res) => {
            const body = req.body;
            if (!EndpointUtils.checkAndSend(body, CUSTOM_FLAVORS_GET_ALL, res))
                return;
            const jwt = body.jwt;
            const jwtData = JWTUtils.checkJWTAndResponse(jwt, res);
            if (!jwtData)
        ...
        app.post("/customFlavors/updateVisibility", async (req, res) => {
            const body = req.body;
            if (!EndpointUtils.checkAndSend(body, CUSTOM_FLAVORS_UPDATE_VISIBILITY, res))
                return;
            const jwt = body.jwt;
            const jwtData = JWTUtils.checkJWTAndResponse(jwt, res);
            if (!jwtData)
