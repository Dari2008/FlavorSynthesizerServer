...
import { USERS_LOGIN_BODY, USERS_REGISTER_BODY } from "../../@types/ApiBodyFormats/Users.js";
...
export default class UsersEndpoints {
    ...
    init(app) {
        app.post("/users/login", async (handle, res) => {
            const body = handle.body;
            if (!EndpointUtils.checkAndSend(body, USERS_LOGIN_BODY, res))
                return;
        ...
        app.post("/users/register", async (handle, res) => {
            const body = handle.body;
            if (!EndpointUtils.checkAndSend(body, USERS_REGISTER_BODY, res))
                return;
            const username = body.username;
            const password = body.password;
