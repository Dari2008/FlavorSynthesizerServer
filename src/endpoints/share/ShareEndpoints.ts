...
import { APIResponse, OpenShareResponse, ShareErrorResponse, ShareResponse } from "../../@types/Api";
import EndpointUtils from "../utils/EndpointUtils";
import { ShareEndpointBody, ShareEndpointOpenBody, ShareEndpointSetDishStateBody } from "../../@types/Endpoints";
import { OPEN_SHARE_BODY, SET_DISH_STATE_BODY, SHARE_BODY } from "../../@types/ApiBodyFormats/Users";
import ShareManager from "../../sql/ShareManager";
import { JWTData, JWTUtils } from "../../utils/Utils";
...
export default class ShareEndpoints {
    ...
    private init(app: Express) {
        ...
        app.post<string, any, APIResponse<ShareResponse, ShareErrorResponse>>("/share/share", async (req, res) => {
            const body = req.body as ShareEndpointBody;
            if (!EndpointUtils.checkAndSend(body, SHARE_BODY, res)) return;
            ...
            let jwtData: JWTData | null | -1 = !!jwt ? JWTUtils.getData(jwt) : -1;
        ...
        app.post<string, any, APIResponse<{}>>("/share/setDishState", async (req, res) => {
            const body = req.body as ShareEndpointSetDishStateBody;
            if (!EndpointUtils.checkAndSend(body, SET_DISH_STATE_BODY, res)) return;
        ...
        app.post<string, any, APIResponse<OpenShareResponse>>("/share/open", async (req, res) => {
            const body = req.body as ShareEndpointOpenBody;
            if (!EndpointUtils.checkAndSend(body, OPEN_SHARE_BODY, res)) return;
